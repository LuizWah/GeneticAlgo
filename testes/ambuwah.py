...
NUMERO_AMBU_TIPO_A = 4
NUMERO_AMBU_TIPO_B = 4
...
def create_solution():
    ...
    new_solution.number_type_B = NUMERO_AMBU_TIPO_A 
...
def calc_rank_aux(solution): 
    ...
    for i in range(NUMBER_POSIBLE_LOCATIONS):
        if((solution.xB[i] == solution.xA[i] == '1' or solution.xB == '1') and (PROBLEMA[i].demand != "nan")):#condition to satisfy
            for j in range(NUMBER_POSIBLE_LOCATIONS):
                if(DISTANCE_MATRIX[i][j] <= RAIO):
                    if(solution.xB[i] == solution.xA[i] == '1'):
